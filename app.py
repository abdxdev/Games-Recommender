...
def run_app():
    ...
    if page == "Home":
        ...
        cols = st.columns(3)
        metrics = [
            ("Total Games", f"{len(data['games']):,}"),
            ("Total Users", f"{len(data['users']):,}"),
            ("Total Recommendations", f"{len(data['recommendations']):,}")
        ]
        ...
        cols = st.columns(2)
        plots = [
            ('positive_ratio', 'Distribution of Positive Ratio', 'Positive Ratio (%)'),
            ('price_final', 'Distribution of Game Prices', 'Price ($)')
        ]
        ...
        for i, (field, title, xlabel) in enumerate(plots):
            with cols[i]:
                ...
                sns.histplot(data_to_plot, bins=20, kde=True, ax=ax)
                ax.set_title(title)
        ...
        cols = st.columns(3)
        platforms = [
            ("Windows Support", data['games']['win'].mean() * 100),
            ("Mac Support", data['games']['mac'].mean() * 100),
            ("Linux Support", data['games']['linux'].mean() * 100)
        ]
                ...
                if not filtered_games.empty:
                    ...
                    metrics = [
                        ...
                        [
                            ...
                            ("Platforms", ", ".join([p for p, v in 
                                zip(["Windows", "Mac", "Linux"], 
                                    [game_details['win'], game_details['mac'], game_details['linux']]) if v])),
                ...
                with st.spinner("Evaluating recommendations for user..."):
                    if user_id in recommendation_models.data['recommendations']['user_id'].values:
                        ...
                        results_df = pd.DataFrame({
                            ...
                            'Hit Rate': [evals[m]['hit_rate'] for m in methods.keys()]
                        })
